...
single_model_results_path = script_path + 'keep_results_AUC' + path_sep + single_model_results_name
...
bites_results_input_path = script_path + 'keep_results_AUC' + path_sep + \
...
# this will be the columns in our running AUC dataframe for all the bites
...
for row in range(num_win_sizes):
    ...
    single_model_AUC = single_model_results_df.ix[row,'AUC']
    ...
    bites_summary_path = script_path + 'keep_results_AUC' + path_sep + \
    ...
    for this_bite_file_path in bites_results_input_filenames:
        ...
        this_bite_AUC = roc_auc_score(y_true,this_bite_probas)
        ...
        this_window_size_running_AUC = roc_auc_score(y_true,this_window_size_running_probas)
        

        # all window sizes running AUC
        ...
        all_window_size_running_AUC = roc_auc_score(y_true,all_running_probas)        
        ...
        this_running_df.ix[this_running_row,'IM AUC'] = single_model_AUC
        this_running_df.ix[this_running_row,'running AUC'] = this_window_size_running_AUC      
        this_running_df.ix[this_running_row,'bite AUC'] = this_bite_AUC 
        ...
        all_running_df.ix[all_running_row,'IM AUC'] = single_model_AUC
        all_running_df.ix[all_running_row,'running AUC'] = all_window_size_running_AUC      
        all_running_df.ix[all_running_row,'bite AUC'] = this_bite_AUC    
    ...
    this_window_size_stacked_AUC = this_running_df.ix[(this_running_row - 1),'running AUC']
    this_running_df.ix[:,'stacked AUC'] = this_window_size_stacked_AUC
    ...
    plot_save_path = script_path + 'keep_results_AUC' + path_sep + 'plots_and_tables' + path_sep
    ...
    this_running_df = this_running_df[this_running_df['IM AUC'] > 0]


    
    # get the extra stuff we need to plot
    single_model_AUC = this_running_df['IM AUC'].as_matrix()[-1]

    this_running_df['sample size %'] = sample_size
    this_running_df['model'] = model
    this_running_df['window size'] = window_size
    
    stacked_AUC = this_running_df['stacked AUC'].as_matrix()[-1]

    min_AUC = this_running_df['bite AUC'].min()
    
    this_running_df['min bite AUC'] = min_AUC
    
    max_AUC = this_running_df['bite AUC'].max()
    
    this_running_df['max bite AUC'] = max_AUC
...
all_window_size_stacked_AUC = all_running_df.ix[(all_running_row - 1),'running AUC']
all_running_df.ix[:,'stacked AUC'] = all_window_size_stacked_AUC
...
stacked_AUC = all_running_df['stacked AUC'].as_matrix()[-1]

min_AUC = all_running_df['bite AUC'].min()

all_running_df['min bite AUC'] = min_AUC

max_AUC = all_running_df['bite AUC'].max()

all_running_df['max bite AUC'] = max_AUC
...
extra_ticks = [stacked_AUC, min_AUC, max_AUC]
...
#sns.lmplot(x = 'sample #', y = 'this file AUC', data =bites_training_df, ci=None,lowess = True)

# sns.rugplot(bites_training_df['this file AUC'])
...
plot_patches_training_df['single model_AUC'] = single_ET_model_array
new_headers = ['stacked AUC', 'sample AUC', 'train file', 'patch #', 'IM AUC']
...
plot_patches_training_df.plot(kind='scatter', 
                            ...
                            label = 'sample AUC',
                            color = 'darkorange')
plot_patches_training_df.plot(x = 'patch #', y = 'stacked AUC', ax = ax1)
plot_patches_training_df.plot(x = 'patch #', y = 'IM AUC', ax = ax1)
